...
\begin{multicols}{2}
	...
	\tbx{ Every $ A_{m\tm n}=PR_{m\sim n} $ for Row reduced Echelon form $ R $and an invertible matrix $ P $ let this relation be denoted by $ A \w{\small{ rrec }} R$}
   ...
   \tbx[Echelon Form]{ every $ A_{m\tm n}=P_m R Q_n $ for $ P,Q $ invertible and $ R $ is such that it has an identity in upper corner and all other entries zero i.e. $ R= \begin{bmatrix}
	...
	\yi Intersection of subspaces (arbitrary) is again a subspace}
	\tbx[Span]{ if $ K=\{v_1,v_,\ck, v_n\} \subseteq V(\F)$ then span of $ K$ is the set $ \{\sum \alpha_iv_i| v_i \in K, \alpha_i \in \F\} $ i.e. is all the formal sums from set $ K $ with $ \F. $ 
	\y given any $ K\subseteq V(F) $ $ span(K) $ is a subspace of $ V(\F) .$ }
	...
	\y The number of elements in Basis is constant $ n\in \Z^+. $ 
	...
	\[[x]_{\beta'}=P[x]_\beta\] where $ P_n $ is a invertible matrix given by if $ b'_j=p_{1j}b_1+p_{2j}b_2+\ck +p_{nj}b_n$ then $ [p_{1j}\; p_{2j} \ck p_{nj}]^T $ forms the $ j^{th} $ column of $ P. $}
	\section{Linear Transform}
	\tbx[Definition]{ a map $ T:V(\F)\ra W(\F) $ (between vectorspaces with same underlying field)  is called a linear transform if for every $ v,u\in V $ and $ \alpha\in F $ 
	\y $ T(v+u)=T(v)+T(u) $ 
	\yi $ T(\alpha v)=\alpha T(V) $}
	\tbx[Range and Null space]{ For a linear transform $ T:V\ra W $ :\\
	\y Rangespace of $ T $ denoted by $ R(T)\subseteq W$ is $ \{w|w=T(v)\w{ for some }v\in V\} $
	\yi NullSpace of $ T $ denoted by $ N(Y)\subseteq V $ is $ \{v|T(v)=0\in W\} $
	...
	\tbx[Rank Nullity Theorem]{for a linear transform  $ T:V(\F)\ra W(\F) $ $ rank(T)=dim(R(T)) $ $ nullity(T)=dim(N(T)) $ and   \[ rank(T)+nullity(T)=dim(V)\]
 ...
 \yi subspaces $ W_1=W_1 $ iff $ W_1^o=W_2^o$ and $ (W_1+W_2)^o=W_1^o\cap W_2^o.$ 
 \yi if $ W $ is subspace of $ V $ then 
 \[dim(W)+dim(W^o)=dim(V)\] 
 \yi if $ W $ is subspace of $ V $ then $ W=W^{oo} .$ }
...
\tbx{ Some Properties need for such a function is :
\yi It must be a linear in terms of rows (or columns) of the matrix this is called $ n $-linear.
\yi It must be alternating i.e. if any 2 rows (or columns )are equal the it is zero.
...
\[E_j(A_n)=\sum\limits_{i=1}^{n}a_{ij}D(A_{ij})\] for fixed $ j\in \{1,2,\ck , n\} $, where $ a_ij $ is the $ i^{th} $ row $ j^{th} $ column entry of $ A $ and $ A_ij $ is the $ n-1\tm n-1 $ matrix obtained from $ A_n $ by removing $ i^{th} $ row and $ j^{th} $ column.}
...
\tbx{ For linear operator $ T\in L(V) $ a vector $ \alpha\in V$ is called an eigenvector and $ \lambda $ called eigenvalue if $ T(\alpha)=\lambda\alpha. $ i.e. $ \alpha \in N(A-\lambda I) $ }
...
\tbx[Diagonalizability test]{ $ T\in L(V) $ is diagonalisable if there exist an ordered basis $ \beta=\{b_1,b_2,\ck, b_n\} $ of $ V $ such that each of the vector in $ \beta $  is an eigenvector of $ T. $   
\yi T is diagonalizable iff characteristic polynomial of $ T $ splits in the underlying field and for each eigenvalue $ \lambda $ of $ T $ the multiplicity (in characteristic polynomial)  equals $ n-rank(T-\lambda I) .$ }
