...
\begin{multicols}{2}
	...
	\tbx{\begin{center}
			...
			Basic $\ra$ in headings indicates easy to derive properties.\\
		\end{center}
	}
	...
	\tbx[ G.C.D ]{ greatest common divisor : $(a,b)=d$ i.e. $gcd(a,b)=d$ if $d|a,d|b$ and if 
		there exist $e$
		such that $e|a,e|d$ then $e|d$ ({\footnotesize here $a,b,m,e \in \Z^+$)}
	} 
	\tbx[ L.C.M ]{ $lcm(a,b)=c$ then $a|c,b|c$ and if there exist $e$ such that $a|e,b|e$ then
		$c|e$ (here $a,b,m,e \in \Z^+$)
	} 
...
\snote{use $ \psi((ab)^{-1})= ab ,\; \psi(b^{-1}a^{-1})=ba$}	}
	...
	\tbx{Any two cyclic groups of same finite order are isomorphic and a cyclic group of infinite order
		is isomorphic to $\Z.$ So cyclic group of order $ n $ an be concidered as $ Z_n=\Z/n\Z $.
	}
	...
	\tbx{for $H=\langle x \rangle $ :
			...
			\y   if $|x|=n<\infty$ the $H=\langle x^a \rangle$ iff $(n,a)=1$ \\ 
...
\tbx{\y Every Group is union of its cyclic groups \snote{as $ a\in G\implies \gen{a}\ll G $ so $ G=\underset{a\in G}{\cup}
		\gen{a} $ .} \\
	\y So if $ |G| $ is infinite it at-least has infinite subgroups.}
	...
	\tbx{ 
		...
		\\$ \gen{A}=\{a_1^{i_1}a_2^{i_2}\ck a_k^{i_n} | a_j \in A, i_l =\pm 1, k\leq n \}$
	...
	\tbx{$G $ is finite solvable group with solvable chain 
		...
		\y   all composition factors of $ G $ are of prime order. \snote{  as $ G/N $ cyclic makes $ G/N $ abelian. } 
		
	}
	\tbx{if $ G $ is a finite group in which every proper subgroup is abelian then $ G $ is solvable.}
	\newpage
	
	\section{Major Theorems and properties}
	
	\tbx[\textbf{ Lagrange's Theorem } ]{if $ G $ is a finite group and $ H $ is a subgroup of $ G $ 
		then :
		
			\y  order of $ H $ divided order of $ G $ \\
	...
	\tbx[\textbf{The Class Equation} ]{for a group $ G $ a finite group if $ g_1,g_2,\ck , g_r $ are representatives of distinct conjugacy classes ({\footnotesize conjugate relation by $ G $ acting on itself by conjugation  )} of $ G $ not contained in 
		$ Z(G) $ then 
		\begin{center}
	...
	\tbx[Consequences of of Sylow's Theorem ]{
		if Sylow $ p $-subgroups of $ G $ : $P$ is normal in $ G $ iff $ P $ char $ G .$ iff all subgroups generated by 
	...
	\tbx[group $ G $ of order $ p^2q $]{
			\y  if $ q<p$ then the Sylow $ p $-subgroup is normal in $ G $ (as $ n_p|q $). \\
	...
	\tbx{if $ H \trianglelefteq G$ (group) then $ G $ acts by conjugation on $ H $ as automorphism of $ H $ i.e.
		$ \sigma_g: H\ra gHg^{-1} $ by $ \sigma_g(h)=ghg^{-} $ is an automorphism. 
	}
	...
	\tbx{clearly $ K \trianglelefteq N_G(K) $ so we have \\
		$ N_G(K)/C_G(K) \cong $ a subgroup of $ Aut(H) .$\\
		Particularly $ G/Z(G) \cong $ a subgroup of $ Aut(G) .$
	}
	\tbx{the preceding mentioned subgroup of $ Aut(G) $ formed by conjugation inner automorphism of $ G $ denoted by 
		$ Inn(G) .$
	}
...
\y thus we have $ D_2n\cong Z_n\smdp Z_2 $ }
	...
	\tbx{each bijection $\sigma$ of $\Omega$ can be represented as the following :
		\begin{center}
			$\sigma=(a_1,a_2,\ck,a_{m_1})(a_{m_1+1},a_{m_1+2},\ck,a_{m_2})\ck$ 
			\\$(a_{m_k+1},a_{m_k+2},\ck,a_{m_{k+1}})$
		\end{center}
		which can be read as (if $\ra$ indicates maps to) 
	\begin{center}
		\begin{tikzpicture}[scale=.515]
		...
		\coordinate (xp) at (4.5,0);
		\node (am1) at  ($(a1)+(xp)$)  {\scriptsize$a_{m_1+1}$};
		\node (am2)  at ($(a2)+(xp)$) {\scriptsize$a_{m_1+2}$};
		\node (amm)  at ($(am)+(xp)$) {\scriptsize$a_{m_2}$};
		\draw[thick,->](am1)  to[bend left=21] (am2);
		\draw [thick,->] (am2) to[bend left=11]  ($(2,0)+(xp)$);
		\draw[thick,loosely dotted] ($(2,0)+(xp)$) arc [radius = 2, start angle = 0, end angle =-180];
		\draw[thick,->] ($(-2,0)+(xp)$)  to[bend left=9] (amm);
		\draw[thick,->](amm)  to[bend left=21] (am1);
		
		\node at (7.1,0) {\footnotesize$,\ck,$};
		
		\coordinate (xp1) at ($2*(xp)+(.7,0)$);
		\coordinate (xpp) at ($(xp)+(.7,0)$);
		\node (b1) at  ($(am1)+(xpp)$)  {\scriptsize$a_{m_k+1}$};
		\node (b2)  at ($(am2)+(xpp)$) {\scriptsize$a_{m_k+2}$};
		\node (bm)  at ($(amm)+(xpp)$) {\scriptsize$a_{m_{k+1}}$};
		\draw[thick,->](b1)  to[bend left=21] (b2);
		\draw [thick,->] (b2) to[bend left=11]  ($(2,0)+(xp1)$);
		\draw[thick,loosely dotted] ($(2,0)+(xp1)$) arc [radius = 2, start angle = 0, end angle =-180];
		\draw[thick,->] ($(-2,0)+(xp1)$)  to[bend left=9] (bm);
...
\end{center}
	...
	\tbx{from preceding point we have, if $ \mathcal{ K } $ is a transposition conjugacy class of $ S_n $ and $ \mathcal{ K }' $ is any other conjugacy class on an element of order $ 2 $ that is not a transposition of $ S_n $ then except for $ n=6 \; (S_6)$ $ |K|\neq |K'| $ (for $ S_6 $ $ |K|=15 $ and for class $ 3 $ disjoint transposition $ |K'|=15 $ only exception).
	}
	...
	\y order of element is equal to the the gcd or lcm of lengths of disjoint cycles \\
...
\tbx{ 
	...
	$ SL(F) =\{A_{n\tm n}=[a_{ij}]|| a_{ij}\in F, Det(A)=0\}$ }
	\tbx{ if $ |F|=q<\infty  $ then 
...
\y every proper subgroup of $ Q $ has infinite index so same goes for $ Q/Z $ \\
...
\y every coset of $ Z $ in $ Q $ contains only one representative $ q\in Q $ in the range $ 0\leq q<1 $ \snote{use if $ 0\leq p,q< $ and $ p+Z=q+Z $ then $ p\in q+Z $ so $ p=q+n $ for $ n\in Z $ then clearly only possible $ n $ is $ 0 $} i.e.\\
...
\snote{use fact that for a $ p $-group center is not $ \{1\} $ and order argument }	}
	...
	\tbx{Properties of Commutators and their generated groups :\\
		if $ H\ll G $ group, $ G'=\gen{[x,y]|\forall x,y\in G} $ and $ x,y\in G $ then 
			...
			so $ G' $ Char $ G .$ \\
			\y  $ G/G' $ is the largest abelian quotient group of $ G $ i.e. if $ H\trianglelefteq G $ and $ G/H $ is normal then $ G'\ll H $ and conversely if $ G'\ll H $ then $ H \trianglelefteq G $ and $ G/H $ is abelian. \\ 
