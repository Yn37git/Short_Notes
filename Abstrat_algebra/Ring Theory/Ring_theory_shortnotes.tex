...
\begin{document}
	...
	\begin{multicols}{2}
		\section{Basic Definitions}
			...
			\y $ \tm $ is associative \snote{$ a\tm (b \tm c)=(a\tm b)\tm c  $} \\
			\y $ \tm $ distributes over $ + $ \snote{$(a+b)\tm c= (a\tm c)+(b\tm c)$}.} 
...
\tbx{if $S,T $ subgrings of $ R $ then :\\
   ...
   \y $ S $ is subring of $ T $ subring of $ R $ then $ S $ subring of $ R. $
			} 
...
\tbx{\textbf{Ideal Generated} by $ A  \subseteq R $ $ (A) $ is the smallest ideal of $ R $ ring containing $ A .$
			} 
...
\tbx{\textbf{if a non zero finite commutative ring has no zero divisors then it is field} \\
			\snote{note : identity being in the ring becomes redundant.\\
				use $ a^n	\in R $ for every $ a \in R, n\in \Z^+$ so is equal to identity for some $ n $ as $ R $ is finite}
			} 
...
\tbx{from above point we get : Let $ R $ be a U.F.D. , $ F $ its field of fractions if $ p(x)\in R[x] $ is a monic polynomial reducible in  $F[x]  \st p(x) =a(x)b(x)$ where $ a(x),b(x)$ are monic and if $ a(x) \notin R[x]$ then 
			$ R[x] $ is not a U.F.D.  
			} 
